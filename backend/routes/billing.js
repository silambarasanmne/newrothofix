...
router.post('/sale', authenticateToken, requireRole('Billing Worker', 'Billing Manager'), (req, res) => {
  try {
    const {
      items, customer_name, customer_phone, customer_address,
      discount_type, discount_value, payment_method, amount_received,
      prescription_id, consultation_id, consultation_charges = 0, other_charges = 0, doctor_name, doctor_id
    } = req.body;

    const targetConsultationId = consultation_id || prescription_id || null;

    // Check Duplicate Billing for the same consultation
    if (targetConsultationId) {
      const existingBill = db.prepare('SELECT id, invoice_number FROM sales WHERE consultation_id = ?').get(targetConsultationId);
      if (existingBill) {
        return res.status(400).json({
          success: false,
          message: `Duplicate Billing Error: Consultation #${targetConsultationId} has already been billed under Invoice #${existingBill.invoice_number}.`
        });
      }
    }
    ...
    // Retrieve Doctor Info from Prescription/Consultation if available
    let resolvedDoctorId = doctor_id || null;
    let resolvedDoctorName = doctor_name || 'Dr. Specialist';

    if (targetConsultationId) {
      const pRecord = db.prepare('SELECT doctor_id, doctor_name FROM prescriptions WHERE id = ?').get(targetConsultationId);
      if (pRecord) {
        if (pRecord.doctor_id) resolvedDoctorId = pRecord.doctor_id;
        if (pRecord.doctor_name) resolvedDoctorName = pRecord.doctor_name;
      }
    }
    ...
    // 2. Discount & Charges Calculation
    const consultFee = parseFloat(consultation_charges || 0);
    const extraFee = parseFloat(other_charges || 0);
    const medicineSubtotal = calculatedSubtotal;
    const overallSubtotal = medicineSubtotal + consultFee + extraFee;
    ...
    if (discountAmt > overallSubtotal) {
      return res.status(400).json({ success: false, message: 'Discount cannot exceed total bill amount.' });
    }

    const grandTotal = overallSubtotal - discountAmt;
    ...
    try {
      ...
      if (targetConsultationId) {
        db.prepare("UPDATE prescriptions SET status = 'Billed' WHERE id = ?").run(targetConsultationId);
      }

      db.exec('COMMIT;');

      logAudit(req, req.user, 'BILL_CREATE', 'BILLING', 'Sale', saleId, `Biller '${req.user.full_name}' generated Invoice #${invoiceNumber} (Total: ₹${grandTotal.toFixed(2)}) for doctor '${resolvedDoctorName}'`);
