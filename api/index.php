...
// Robustly strip subfolders before /api/ to get the true endpoint
...
if ($uri === '/patients' && $method === 'POST') {
    ...
    $age = isset($body['age']) ? intval($body['age']) : null;
    $mobile = trim($body['mobile'] ?? '');
    $symptoms = trim($body['symptoms'] ?? '');

    if (!$patient_name || $age === null || !$mobile || !$symptoms) {
        sendJson(['success' => false, 'message' => 'All fields are required.'], 400);
    }

    $stmt = $pdo->query("SELECT MAX(token) as max_token FROM patients");
    $maxToken = $stmt->fetchColumn() ?: 0;
    $newToken = $maxToken + 1;

    $stmt = $pdo->prepare("INSERT INTO patients (token, patient_name, age, mobile, symptoms) VALUES (?, ?, ?, ?, ?)");
    $stmt->execute([$newToken, $patient_name, $age, $mobile, $symptoms]);
...
if ($uri === '/patients' && $method === 'GET') {
    ...
    if ($search) {
        $query .= " WHERE patient_name LIKE ? OR mobile LIKE ? OR token = ?";
        $s = "%$search%";
        $params = [$s, $s, $search];
    }
...
if (preg_match('#^/patients/(\d+)$#', $uri, $matches) && $method === 'GET') {
    requireAuth();
    $token = $matches[1];
    $stmt = $pdo->prepare("SELECT * FROM patients WHERE token = ?");
    $stmt->execute([$token]);
    $patient = $stmt->fetch();
    
    if ($patient) {
        sendJson(['success' => true, 'data' => $patient]);
    } else {
        sendJson(['success' => false, 'message' => 'No patient found with token #'.$token], 404);
    }
}
...
if ($uri === '/stats' && $method === 'GET') {
    ...
    $today_count = $pdo->query("SELECT COUNT(*) FROM patients WHERE date(created_at) = '$today'")->fetchColumn();
    sendJson(['success' => true, 'total_patients' => intval($total), 'today_patients' => intval($today_count)]);
}

// -----------------------------------------------------------------------------
// 7. PRESCRIPTIONS ROUTING
// -----------------------------------------------------------------------------
if ($uri === '/prescriptions' && $method === 'POST') {
    $user = requireAuth();
    $patient_token = $body['patient_token'] ?? null;
    ...
    if (!$patient_token) {
        sendJson(['success' => false, 'message' => 'Patient token is required.'], 400);
    }
    if (!is_array($items) || count($items) === 0) {
        sendJson(['success' => false, 'message' => 'Prescription must contain at least one medicine.'], 400);
    }

    try {
        $pdo->beginTransaction();

        $stmt = $pdo->prepare("INSERT INTO prescriptions (patient_token, patient_mobile, doctor_name, status) VALUES (?, ?, ?, 'Pending')");
        $stmt->execute([$patient_token, $patient_mobile, $user['full_name']]);
        $prescriptionId = $pdo->lastInsertId();

        $stmtCons = $pdo->prepare("INSERT INTO consultations (prescription_id, patient_token, patient_name, patient_mobile, age, symptoms, doctor_comment) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmtCons->execute([$prescriptionId, $patient_token, $patient_name, $patient_mobile, $age, $symptoms, $doctor_comment]);

        $stmtItem = $pdo->prepare("INSERT INTO prescription_items (prescription_id, medicine_id, medicine_name, quantity, instructions) VALUES (?, ?, ?, ?, ?)");
        foreach ($items as $item) {
            $stmtItem->execute([
                $prescriptionId,
                $item['medicine_id'],
        ...
        $pdo->commit();
        sendJson(['success' => true, 'message' => 'Prescription saved successfully.', 'prescription_id' => $prescriptionId]);
    } catch (Exception $e) {
        $pdo->rollBack();
        sendJson(['success' => false, 'message' => 'Failed to save prescription: ' . $e->getMessage()], 500);
    }
}

if ($uri === '/prescriptions/today' && $method === 'GET') {
    requireAuth();
    $today = date('Y-m-d');
    $stmt = $pdo->prepare("
    ...
    ");
    $stmt->execute([$today]);
    sendJson(['success' => true, 'prescriptions' => $stmt->fetchAll()]);
}

if (preg_match('#^/prescriptions/patient/([^/]+)$#', $uri, $matches) && $method === 'GET') {
    requireAuth();
    ...
    if (!$prescription) {
        sendJson(['success' => false, 'message' => 'No pending prescription found for this patient.']);
    }
    ...
    sendJson(['success' => true, 'prescription' => $prescription]);
}
